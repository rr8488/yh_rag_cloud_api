...
# ---
# 3. DEFINE PARSER-SPECIFIC HELPERS
# ---

def _parse_deliverables_section(report_data: Dict, find_answer: callable) -> Dict:
    """
    Parses the deliverables section, which is common to BOTH
    mid-progress and final reports.
    """
    deliverables_list = []
    # Use flexible regex to find "number of deliverable"
        ...
        try:
            ...
            for i in range(1, num_deliverables + 1):
                ...
                if not desc and not update:
                    # Check for GSS Final Report format which can be a single block
                    if i == 1 and num_deliverables == 7:  # Specific to GSS-5-final.pdf
                        desc = find_answer(r"state deliverable #1")
                        update = find_answer(r"Update for deliverable #1")
                        if not desc and not update: continue
                    else:
                ...
                # Handle cases where description is in the update block (GSS Final Report)
                if not desc and update:
                    match = re.search(r"(#[^#]+)\s*Update\s*:\s*#", update, re.I)
                    if match:
...
def _parse_mid_progress_sections(report_data: Dict, find_answer: callable) -> Dict:
    """
...
# ---
# 4. DEFINE THE MAIN PARSER FUNCTION (HTML-BASED)
# ---
...
def parse_progress_report(raw_report_content: str) -> Dict[str, Any]:
    ...
    if is_final_report:
        ...
        report_data = _parse_mid_progress_sections(report_data, find_answer)

    # ---
    # 7. PARSE COMMON SECTIONS (Deliverables)
    # ---

    # The deliverables section is common to both report types.
    report_data = _parse_deliverables_section(report_data, find_answer)
    ...
    try:
        ...
        print(
            f"[parse_progress_report] HTML Q&A parsing and validation successful. "
