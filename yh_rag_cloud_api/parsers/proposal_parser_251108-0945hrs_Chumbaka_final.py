...
def _clean_content(text: str, remove_bilingual=True) -> str:
    ...
    # --- FIX V39.2 (was V39.1) ---
    # This rule is now conditional. It only runs during final cleaning,
    # not during classification, to preserve keys like "Location(s)".
    if remove_bilingual:
        text = re.sub(r'\([^)]*\)', '', text, flags=re.DOTALL)
    # --- END FIX ---
    ...
    # --- FIX V39.3 ---
    # Only strip the trailing colon during final cleaning,
    # not during classification/logic passes. This fixes the
    # V39.2 "combine" logic being too aggressive.
    if remove_bilingual:
        text = text.rstrip(':')
    # --- END FIX ---
...
def parse_proposal(proposal_text: str) -> dict:
    ...
    # --- V39.2: Convert to while loop to handle combined headers ---
    i = 0
    while i < len(matches):
        ...
        # --- V39.2: Combine Split Header Logic ---
        combined_tag_match = current_match  # By default, just the current tag
        final_question_text = question_text

        # Check conditions:
        # 1. Current question does NOT have a colon
        # 2. There is a next match
        if ":" not in question_text and (i + 1) < len(matches):
            # Get next question text
            next_question_text = _clean_content(matches[i + 1].group(1), remove_bilingual=False)

            # 3. Next question DOES have a colon
            _nq_q, _nq_a, nq_found_answer_in_tag = _split_at_colon(next_question_text)

            if ":" in next_question_text and nq_found_answer_in_tag:
                # Conditions met! Combine them.
                print(
                    f"    [Parser] V39.2: Combining split question: '{question_text[:30]}...' + '{next_question_text[:30]}...'")
                final_question_text = question_text + " " + next_question_text

                # The "combined tag" is now the *next* match
                combined_tag_match = matches[i + 1]

                # Manually increment i to skip the next tag, it's been processed
                i += 1
                # --- END V39.2 Logic ---

        # 2. Get the Answer (the text *between* our (possibly combined) tag and the next)

        # The start index is the end of the *last* tag we processed
        content_start_index = combined_tag_match.end()

        content_end_index = -1
        # The *next* tag is *always* at i + 1 (because we already incremented i if we combined)
        if (i + 1) < len(matches):
        ...
        # We use the (possibly combined) final_question_text
        ...
        # --- "Last Question" Heuristic (V39.1 - CORRECTED) ---
        if not final_answer:
            # This is a header with no answer (e.g., "Project Goal")
            if header_key != "unclassified" and header_key != "unclassified_answer_marker":
                # It's a *valid* header.

                # If we are *already* holding a key, DON'T overwrite it.
                if last_valid_key is None:
                    last_valid_key = header_key
                    last_valid_question_text = final_question
                    print(f"    [Parser] V39.2: Holding key '{header_key}'")
                else:
                    # This is a chained question header, ignore it
                    print(f"    [Parser] V39.2: Chaining question to held key '{last_valid_key}'")

            # If it's unclassified (or a stray ':'), we just ignore it.

            # --- V39.2: Must increment i before continuing ---
            i += 1
            continue
        ...
        # This is an "answer" tag (like ": This project serves..." or "a) Impact study...")
        # Check if we are "holding" a valid key from a *previous* tag.
        if last_valid_key and (header_key == "unclassified" or header_key == "unclassified_answer_marker"):
            # Yes. This answer belongs to the *previous* held question.
            current_key = last_valid_key
            current_question = last_valid_question_text
            print(f"    [Parser] V39.2: Re-assigning answer to '{current_key}'")
        # --- END V39.1 FIX ---
        ...
        if current_key == "unclassified":
            ...
            print(f"    [Parser] V39.2: Saved '{current_key}'")
        else:
            ...
            print(f"    [Parser] V39.2 (Pass 3) skipping '{current_key}', set by metadata pass.")
